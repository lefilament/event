    ...
    def _process_attendees_form(self, event, form_details):
        ...
        for key, _value in form_details.items():
            ...
            if key.startswith("question_multi_answer"):
                ...
                assert question_sudo.question_type == "multiple_choice"  # otherwise, html is malformed
                answer_values = {
                ...
                # question with registration index are specific to one registration
                    ...
                    registrations[rindex]["registration_answer_ids"].append((0, 0, answer_values))
                    if question_sudo.is_mandatory_answer:
        ...
        for q, c in mandatory_multiple_general_question_answer_count.items():
            if c == 0:
                raise UserError("Question " + str(q) + " is mandatory but did not receive an answer.")
        # specific
        for r, cc in enumerate(mandatory_multiple_specific_question_answer_count):
            for q, c in cc.items():
                if c == 0:
                    raise UserError("Question " + str(q) +
                                    " is mandatory but did not receive an answer in ticket number " + str(r) + ".")
